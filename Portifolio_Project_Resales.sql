...
-- Table geolocation_ds has some values in geolocation_city with not permited symbols, such as Ã and £, therefore, this collumn won`t be used
...
SELECT TOP 100 ord.product_id, COUNT(ord.product_id) OVER (PARTITION BY ord.product_id) AS num_product/*,
...
INNER JOIN distance_table AS dis ON dis.product_id =  ord.product_id
WHERE ord.product_id <> 'NULL'
--GROUP BY ord.product_id, ord.freight_value, ord.price, dis.distance_in_km
